...
public class ShaderManager
{
    ...
    public void SetShaderValue(string name, string uniform, Vec2 value) =>
        SetShaderValue(name, uniform, new Vector2(value.X, value.Y), ShaderUniformDataType.SHADER_UNIFORM_VEC2);
    ...
    public void SetShaderValue<T>(string name, string uniform, T value, ShaderUniformDataType uniformType) where T: unmanaged
    {
        if (_shaders.TryGetValue(name, out var shader))
            Raylib.SetShaderValue(shader, Raylib.GetShaderLocation(shader, uniform), value, uniformType);
        else
    ...
    public void AddShader(string name, Shader shader)
    {
        if(!_shaders.TryAdd(name, shader))
            DebugManager.Log(LogLevel.LogWarning, $"SE_SHADERMANAGER: Shader already exist : {name}");
    }
